...
import { r as register } from '../../../vendor/webworker-promise/lib/register.js';
...
function handlePaintEllipse(_ref2) {
  ...
  var zmin = Math.round(Math.max(center[2] - radius3[2], 0));
  var zmax = Math.round(Math.min(center[2] + radius3[2], globals.dimensions[2] - 1));

  for (var z = zmin; z <= zmax; z++) {
    var dz = (center[2] - z) / radius3[2];
    var ay = radius3[1] * Math.sqrt(1 - dz * dz);
    var ymin = Math.round(Math.max(center[1] - ay, 0));
    var ymax = Math.round(Math.min(center[1] + ay, globals.dimensions[1] - 1));

    for (var y = ymin; y <= ymax; y++) {
      var dy = (center[1] - y) / radius3[1];
      var ax = radius3[0] * Math.sqrt(1 - dy * dy - dz * dz);
      var xmin = Math.round(Math.max(center[0] - ax, 0));
      var xmax = Math.round(Math.min(center[0] + ax, globals.dimensions[0] - 1));
