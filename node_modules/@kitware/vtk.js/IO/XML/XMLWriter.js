import { x as xmlbuilder2_min } from '../../vendor/xmlbuilder2/lib/xmlbuilder2.min.js';
import { f as fromByteArray_1 } from '../../vendor/base64-js/index.js';
...
function processDataArray(dataArray, format, blockSize) {
  ...
  if (format === FormatTypes.BINARY) {
    if (compressor === 'vtkZLibDataCompressor') {
      ...
      return fromByteArray_1(headerUint8) + fromByteArray_1(uint8);
    }
...
function vtkXMLWriter(publicAPI, model) {
  ...
  publicAPI.create = function (dataObject) {
    return xmlbuilder2_min.create().ele('VTKFile').att('type', model.dataType).att('version', '0.1').att('byte_order', 'LittleEndian').att('header_type', 'UInt32').att('compressor', model.format === FormatTypes.ASCII ? '' : 'vtkZLibDataCompressor');
  };
