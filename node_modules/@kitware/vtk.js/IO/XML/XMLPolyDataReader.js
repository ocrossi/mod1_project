...
function handleFieldDataArray(dataArrayElem, compressor, byteOrder, headerType, binaryBuffer) {
  ...
  return vtkDataArray.newInstance(vtkXMLReader.processDataArray(size, dataArrayElem, compressor, byteOrder, headerType, binaryBuffer));
} // ----------------------------------------------------------------------------
...
function vtkXMLPolyDataReader(publicAPI, model) {
  ...
  publicAPI.parseXML = function (rootElem, type, compressor, byteOrder, headerType) {
    ...
    if (fieldDataElem) {
      fieldDataArrays = _toConsumableArray(fieldDataElem.getElementsByTagName('DataArray')).map(function (daElem) {
...
export default vtkXMLPolyDataReader$1;
