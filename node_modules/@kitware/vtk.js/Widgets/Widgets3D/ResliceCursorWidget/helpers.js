...
import { g as subtract, l as normalize, j as cross, P as multiplyAccumulate, R as signedAngleBetweenVectors } from '../../../Common/Core/Math/index.js';
...
function updateLine(lineState, center, axis, lineLength, rotationLength) {
  ...
  var p2 = [center[0] + lineLength * axis[0], center[1] + lineLength * axis[1], center[2] + lineLength * axis[2]];
  var rotationP1 = [center[0] - rotationLength * axis[0], center[1] - rotationLength * axis[1], center[2] - rotationLength * axis[2]];
  var rotationP2 = [center[0] + rotationLength * axis[0], center[1] + rotationLength * axis[1], center[2] + rotationLength * axis[2]];
...
function updateState(widgetState) {
  ...
  var center = widgetState.getCenter(); // Factor used to define where the rotation point will be displayed
  // according to the plane size where there will be visible

  var factor = 0.5 * 0.85;
  var xRotationLength = (bounds[1] - bounds[0]) * factor;
  var yRotationLength = (bounds[3] - bounds[2]) * factor;
  var zRotationLength = (bounds[5] - bounds[4]) * factor; // Length of the principal diagonal.

  var pdLength = 20 * 0.5 * vtkBoundingBox.getDiagonalLength(bounds);
  updateLine(widgetState.getAxisXinY(), center, xyIntersectionLineAxis, pdLength, zRotationLength);
  updateLine(widgetState.getAxisYinX(), center, xyIntersectionLineAxis, pdLength, zRotationLength);
  updateLine(widgetState.getAxisYinZ(), center, yzIntersectionLineAxis, pdLength, xRotationLength);
  updateLine(widgetState.getAxisZinY(), center, yzIntersectionLineAxis, pdLength, xRotationLength);
  updateLine(widgetState.getAxisXinZ(), center, xzIntersectionLineAxis, pdLength, yRotationLength);
  updateLine(widgetState.getAxisZinX(), center, xzIntersectionLineAxis, pdLength, yRotationLength);
}
/**
